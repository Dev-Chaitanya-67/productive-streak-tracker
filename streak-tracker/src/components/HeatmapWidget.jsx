  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const focusLogs = await focusRes.json();
           ...
           <div className="flex bg-zinc-100 dark:bg-zinc-900 p-1 rounded-xl w-full sm:w-auto overflow-x-auto no-scrollbar">
              {TABS.map(tab => (
                <button
                  ...
                  className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all whitespace-nowrap ${
                    activeTab === tab.id 
